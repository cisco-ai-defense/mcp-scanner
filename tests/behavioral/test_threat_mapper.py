...
class TestThreatMapper:
    ...
    def test_behavioral_threat_categories(self):
        ...
        for threat_name, threat_data in behavioral_threats.items():
            ...
            assert (
                "aitech_name" in threat_data
            ), f"Missing aitech_name for {threat_name}"
            assert "aisubtech" in threat_data, f"Missing aisubtech for {threat_name}"
            assert (
                "aisubtech_name" in threat_data
            ), f"Missing aisubtech_name for {threat_name}"
            assert "severity" in threat_data, f"Missing severity for {threat_name}"
            assert (
                "scanner_category" in threat_data
            ), f"Missing scanner_category for {threat_name}"
    ...
    def test_specific_threat_categories(self):
        ...
        for threat_name in expected_critical_threats:
            assert (
                threat_name in behavioral_threats
            ), f"Missing critical threat: {threat_name}"
            assert (
                behavioral_threats[threat_name]["severity"] == "HIGH"
    ...
    def test_scanner_categories_valid(self):
        ...
        for threat_name, threat_data in behavioral_threats.items():
            ...
            assert (
                scanner_cat in valid_categories
    ...
    def test_behavioral_threats_count(self):
        ...
        assert (
            len(behavioral_threats) >= 14
        ), f"Expected at least 14 threats, got {len(behavioral_threats)}"
    ...
    def test_data_exfiltration_threat_details(self):
        ...
        assert (
            "extracting" in threat["description"].lower()
            or "exfiltration" in threat["description"].lower()
        )
