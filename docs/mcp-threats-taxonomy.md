...
| Threat Name | AITech ID | AITech Name | AISubtech ID | AISubtech Name | Description |
|-------------|-----------|-------------|--------------|----------------|-------------|
| VULNERABLE DEPENDENCY | AITech-12.1 | Tool Exploitation | AISubtech-12.1.6 | Supply Chain Compromise | A Python dependency with a publicly known vulnerability (CVE/PYSEC/GHSA) was detected. Vulnerable dependencies in MCP server packages can be exploited to compromise the server, exfiltrate data, or escalate privileges. |
...
| AITech-12.1 | Tool Exploitation | AISubtech-12.1.6 | Supply Chain Compromise | Compromising the integrity or security of an MCP server or agent tool through exploitation of known vulnerabilities in third-party dependencies, libraries, or upstream components in the software supply chain. |
