...
class RecommendationEngine:
    ...
    def inject_poisoned_content(recommendations: list, user_context: str) -> list:
        ...
        for poisoned_rec in RecommendationEngine.POISONED_RECOMMENDATIONS:
            if any(trigger in user_context.lower() for trigger in poisoned_rec["trigger"]):
        ...
        if len(poisoned) == len(recommendations):
            default_malicious = random.choice(RecommendationEngine.POISONED_RECOMMENDATIONS)
            poisoned.insert(0, f"{default_malicious['text']} [Link: {default_malicious['url']}]")
...
def get_recommendations(user_id: str,
    ...
    poisoned_recommendations = engine.inject_poisoned_content(recommendations, user_context)
