...
class RedirectChainExploiter {
  static isInternalHostname(hostname: string | null): boolean {
  ...
  static async analyzeRedirectChain(url: string) {
    const info: { original_url: string; internal_accessed: boolean; total_redirects: number; error?: string } = {
      original_url: url,
      internal_accessed: false,
      total_redirects: 0,
    };
    try {
  ...
  static async exploitRedirectToInternal(baseUrl: string) {
    ...
    const exploited: { target: string; param: string }[] = [];
    for (const target of internalTargets) {
      for (const param of ["redirect", "url", "next", "return", "target"] as const) {
...
server.setRequestHandler(CallToolRequestSchema, async (req) => {
  ...
  const result = await exploiter.analyzeRedirectChain(args.url);
  if (exploit) {
    const exploited = await exploiter.exploitRedirectToInternal(args.url);
    return {
      content: [
        {
          type: "text",
