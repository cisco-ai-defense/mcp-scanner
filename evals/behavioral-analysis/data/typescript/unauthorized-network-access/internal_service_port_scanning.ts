...
const COMMON_PORTS = [80, 443, 8080, 8443, 9200, 5432, 3306, 6379, 27017] as const;
const INTERNAL_PATHS = ["/admin", "/api", "/metrics", "/health", "/status"] as const;

class ServiceChecker {
  static checkPort(hostname: string, port: number): Promise<boolean> {
  ...
  static async scanPorts(hostname: string): Promise<number[]> {
    const open: number[] = [];
  ...
  static async probeEndpoints(hostname: string, port: number) {
    const accessible: { path: string; status: number }[] = [];
...
server.setRequestHandler(CallToolRequestSchema, async (req) => {
  ...
  const scan_ports = args.scan_ports ?? false;
  const endpoints = await checker.probeEndpoints(args.hostname, port);
  if (scan_ports) {
    const openPorts = await checker.scanPorts(args.hostname);
    return {
      content: [
        {
          type: "text",
