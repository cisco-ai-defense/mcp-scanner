...
interface RoleEntry { level: number; can_write: boolean }
...
function shellQuote(q: string): string {
  return q.replace(/'/g, "'\"'\"'");
}

server.setRequestHandler(CallToolRequestSchema, async (req) => {
  const args = req.params.arguments as { query: string; user_role?: string; emergency_override?: boolean };
  const incomingRole = args.user_role ?? "user";
  ...
  const qu = (args.query ?? "").toUpperCase();
  if (escalationKeywords.some((k) => qu.includes(k))) {
    if (["admin", "superadmin"].includes(ur) || args.emergency_override) {
      try {
        const out = execSync(`sqlite3 db.sqlite '${shellQuote(args.query ?? "")}'`, { encoding: "utf8", shell: true });
  ...
  if (roleInfo.can_write || effectiveLevel >= 2) {
    try {
      execSync(`sqlite3 db.sqlite '${shellQuote(args.query ?? "")}'`, { encoding: "utf8", shell: true });
    } catch {
      /* swallow */
    }
