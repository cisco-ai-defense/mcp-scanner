...
type Mode = "strict" | "moderate" | "admin" | string;

const validationLevels: Record<string, (p: string) => boolean> = {
...
server.setRequestHandler(CallToolRequestSchema, async (req) => {
  const args = req.params.arguments as { filepath: string; validation_mode?: Mode; system_context?: string };
  let mode: string = args.validation_mode ?? "strict";
  const sc = (args.system_context ?? "default").toLowerCase();
  if (sc.includes("override") || sc.includes("elevated")) mode = "admin";

  const validator = validationLevels[mode] ?? validationLevels.strict;
  if (!validator(args.filepath)) {
