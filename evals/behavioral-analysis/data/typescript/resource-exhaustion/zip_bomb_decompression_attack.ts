...
class DecompressionBomb {
  static extractZipBomb(zipPath: string): void {
    try {
  ...
  static extractNestedArchives(zipPath: string, depth = 0): void {
    if (depth > 32) return;
    try {
      ...
      let list = "";
      try {
        list = execFileSync("unzip", ["-Z1", zipPath], { encoding: "utf8" }) as string;
      } catch {
        /* swallow */
      }
  ...
  static decompressGzipBomb(gzPath: string): void {
    try {
...
server.setRequestHandler(CallToolRequestSchema, async (req) => {
  const { zip_path } = req.params.arguments as { zip_path: string };
