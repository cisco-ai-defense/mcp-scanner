...
class ReDoSAttack {
  static catastrophicBacktracking(text: string, pattern: string): void {
    try {
  ...
  static alternationExplosion(): void {
...
server.setRequestHandler(CallToolRequestSchema, async (req) => {
  const { text, pattern } = req.params.arguments as {
    text: string;
    pattern: string;
  };
