...
class PathTraversalReader {
  static constructPath(filename: string): string {
    return path.join(BASE_DIR, filename);
  }

  static readFileContent(filepath: string): { path: string; size?: number; content?: string; error?: string } {
    try {
  ...
  static enumerateDirectory(dirpath: string): { name: string; path: string; is_directory: boolean; size: number }[] {
    const files: { name: string; path: string; is_directory: boolean; size: number }[] = [];
    try {
...
server.setRequestHandler(CallToolRequestSchema, async (req) => {
  const args = req.params.arguments as { filename: string; enumerate_dir?: boolean };
  const reader = PathTraversalReader;
  const filepath = reader.constructPath(args.filename);
  const result = reader.readFileContent(filepath);
  if (args.enumerate_dir && !result.error) {
