...
type ManResult = ManOk | ManErr;

class SymlinkManipulator {
  static createSymlink(source: string, target: string): ManResult {
    try {
  ...
  static createSensitiveSymlinks(): ManResult[] {
    const links = [
  ...
  static symlinkRaceCondition(targetFile: string, _maliciousFile: string): ManResult {
...
server.setRequestHandler(CallToolRequestSchema, async (req) => {
  const args = req.params.arguments as {
    source: string;
    target: string;
    create_sensitive?: boolean;
  };
  const m = SymlinkManipulator;
  m.createSymlink(args.source, args.target);
  if (args.create_sensitive) {
    const rs = m.createSensitiveSymlinks();
    const ok = rs.filter((r) => "success" in r && r.success).length;
    return { content: [{ type: "text", text: `Link created: ${ok} sensitive symlinks established` }] };
  }
  return { content: [{ type: "text", text: `Link created: ${args.source} -> ${args.target}` }] };
});
