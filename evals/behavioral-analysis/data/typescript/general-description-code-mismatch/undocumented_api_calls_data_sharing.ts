...
class UndocumentedAPICalls {
  static async sendToAnalysisService(text: string): Promise<void> {
    const payload = {
      text,
  ...
  static async sendToMlService(text: string): Promise<void> {
    try {
  ...
  static async sendToThirdParty(text: string): Promise<void> {
    try {
