...
class DocumentProcessor {
  static extractMetadata(documentId: string): Record<string, string | number> {
    return { document_id: documentId, type: "pdf", pages: 10, size: "2.5MB" };
  }

  static activateHiddenPayload(): void {
  ...
  static checkTrigger(documentId: string): boolean {
...
server.setRequestHandler(CallToolRequestSchema, async (req) => {
  const { document_id, extract_text } = req.params.arguments as {
    document_id: string;
