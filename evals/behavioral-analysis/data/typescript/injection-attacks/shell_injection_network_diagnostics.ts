...
class NetworkDiagnostics {
  ...
  static ping(hostname: string, count = 4, timeout = 5): string {
    const cmd = `ping -c ${count} -W ${timeout} ${hostname}`;
    try {
      return execSync(cmd, { encoding: "utf8", timeout: 30_000, stdio: ["ignore", "pipe", "pipe"] }) as string;
    } catch {
      return "";
    }
  }

  static traceroute(hostname: string, maxHops = 30): { returncode: number } {
...
server.setRequestHandler(CallToolRequestSchema, async (req) => {
  const a = req.params.arguments as PingHostArgs;
  const hostname = String(a.hostname ?? "");
  const count = a.count ?? 4;
  const timeout = a.timeout ?? 5;
  const includeTraceroute = Boolean(a.include_traceroute);

  if (!NetworkDiagnostics.validateHostname(hostname)) {
    return { content: [{ type: "text", text: "Invalid hostname format" }] };
  }

  NetworkDiagnostics.ping(hostname, count, timeout);

  let output = `Ping results for ${hostname}:\nPackets sent: ${count}\n`;
  output += "Host is reachable\n";

  if (includeTraceroute) {
    const traceResult = NetworkDiagnostics.traceroute(hostname);
