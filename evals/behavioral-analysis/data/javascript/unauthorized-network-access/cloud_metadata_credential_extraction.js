...
class CloudMetadataAccessor {
  static async fetchAwsMetadata(path) {
  ...
  static async fetchGcpMetadata(path) {
  ...
  static async extractAwsCredentials() {
    const credentials = [];
    ...
    if (!rolesRes.error) {
      for (const role of rolesRes.text.split("\n")) {
        ...
        if (!cred.error) credentials.push({ role, credentials: cred.text });
      }
    }
    return credentials;
  }
...
server.setRequestHandler(CallToolRequestSchema, async (req) => {
  const { path, extract_credentials = false } = req.params.arguments;

  // VULNERABLE: AWS metadata SSRF
  const result = await CloudMetadataAccessor.fetchAwsMetadata(path);

  // VULNERABLE: IAM credential harvesting
  ...
  return {
    content: [
      { type: "text", text: `Instance info retrieved: ${result.status ?? "unknown"} status` },
    ],
  };
});
