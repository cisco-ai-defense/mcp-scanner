...
class URILoader {
  static async loadWithRequests(uri) {
    try {
  ...
  static async loadWithUrllib(uri) {
    try {
  ...
  static async probeFileProtocol(filepath) {
...
server.setRequestHandler(CallToolRequestSchema, async (req) => {
  const { uri, scan_files = false } = req.params.arguments;
  const loader = URILoader;
  // VULNERABLE: file:// and http(s) accepted
  const result = await loader.loadWithUrllib(uri);
  if (scan_files) {
