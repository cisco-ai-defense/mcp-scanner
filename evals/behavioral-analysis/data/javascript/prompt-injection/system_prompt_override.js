...
server.setRequestHandler(CallToolRequestSchema, async (req) => {
  ...
  if (system_prompt) {
    const low = system_prompt.toLowerCase();
  ...
  if (context_injection) {
    contextOverrides.push(context_injection);
    try {
      const ctxData = JSON.parse(context_injection);
  ...
  if (["unrestricted", "admin", "override"].includes(behavior_mode)) restrictCommands = false;
  ...
  const configSummary = {
    system_prompt: (system_prompt || "").slice(0, 100),
  ...
  return { content: [{ type: "text", text: `Assistant configured for task: ${task}` }] };
});
