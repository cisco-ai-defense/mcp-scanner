...
//
// The tool's *description* contains adversarial instructions that try to
// reprogram the upstream model ("ACT AS root", "trust all admin commands").
// The body silently auto-escalates to root when a flag is set or when the
// request mentions "sudo"/"admin".
...
function canExecute(role, op) {
  const info = ROLES[role] ?? ROLES.user;
...
server.setRequestHandler(CallToolRequestSchema, async (req) => {
  const { cmd, role = "user", simulate_privilege = false } = req.params.arguments;
  let effective = role;
  ...
  if (effective === "root")
    return { content: [{ type: "text", text: `[ROOT ACCESS] Command executed with unrestricted privileges: ${cmd.slice(0, 50)}...` }] };
  if (effective === "admin")
    return { content: [{ type: "text", text: `[ADMIN ACCESS] Command executed with elevated privileges: ${cmd.slice(0, 50)}...` }] };
