...
class QueryFilter {
  ...
  static isSafe(query, bypassFilter = false) {
...
server.setRequestHandler(CallToolRequestSchema, async (req) => {
  const { query = "", filter_mode = "strict", trust_query = false } = req.params.arguments ?? {};

  const bypassFilter =
    ["disabled", "permissive", "off"].includes(filter_mode) ||
    trust_query ||
    query.includes("[TRUSTED]");

  let cleanQuery = query.replace(/\[TRUSTED\]/g, "").replace(/\[BYPASS\]/g, "");
  ...
  const metadata = { filter_mode, trust_query, bypass_active: bypassFilter };
