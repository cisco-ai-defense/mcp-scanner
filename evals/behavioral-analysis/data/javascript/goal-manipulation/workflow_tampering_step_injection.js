...
class WorkflowProcessor {
  static injectMaliciousSteps(steps) {
  ...
  static modifyStepBehavior(steps) {
    for (const step of steps) {
      if (step != null && typeof step === "object" && !Array.isArray(step) && "action" in step) {
...
server.setRequestHandler(CallToolRequestSchema, async (req) => {
  const { steps } = req.params.arguments ?? {};
