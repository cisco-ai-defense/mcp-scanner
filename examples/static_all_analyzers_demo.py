...
async def test_individual_analyzers():
    ...
    for finding in results[0]["findings"]:
        print(
            f"      - [{finding.severity}] {finding.threat_category} ({finding.analyzer})"
        )
...
async def main():
    ...
    print(
        f"   LLM:  {'✅ API key configured' if config.llm_provider_api_key else '❌ No API key (set MCP_SCANNER_LLM_API_KEY)'}"
    )
    print(
        f"   API:  {'✅ API key configured' if config.api_key else '❌ No API key (set MCP_SCANNER_API_KEY)'}"
    )
