...
class TSScope:
    ...
    def define(
        self, name: str, node: Any, type_info: TSType = None, is_param: bool = False
    ) -> None:
...
class TreeSitterSemanticAnalyzer:
    ...
    def _visit_class(self, node: Node) -> None:
        ...
        if name_node:
            ...
            self.current_scope.define(
                class_name, node, TSType(TSTypeKind.CLASS, name=class_name)
            )
    ...
    def _visit_assignment(self, node: Node) -> None:
        ...
        if target:
            ...
            self.current_scope.define(
                target_name, node, type_info, is_param=uses_params
            )
            ...
            if value and value.type in (
                ...
                func = value.child_by_field_name(
                    "function"
                ) or value.child_by_field_name("type")
    ...
    def _parse_type(self, node: Node) -> TSType:
        ...
        type_map = {
            "int": TSTypeKind.INT,
            "int32": TSTypeKind.INT,
            "int64": TSTypeKind.INT,
            "integer": TSTypeKind.INT,
            "number": TSTypeKind.INT,
            "float": TSTypeKind.FLOAT,
            "float32": TSTypeKind.FLOAT,
            "float64": TSTypeKind.FLOAT,
            "double": TSTypeKind.FLOAT,
            "string": TSTypeKind.STRING,
            "str": TSTypeKind.STRING,
            "bool": TSTypeKind.BOOL,
            "boolean": TSTypeKind.BOOL,
            "void": TSTypeKind.VOID,
            "unit": TSTypeKind.VOID,
            "null": TSTypeKind.NULL,
            "nil": TSTypeKind.NULL,
            "none": TSTypeKind.NULL,
            "any": TSTypeKind.ANY,
            "object": TSTypeKind.OBJECT,
            "array": TSTypeKind.ARRAY,
            "list": TSTypeKind.ARRAY,
            "map": TSTypeKind.MAP,
            "dict": TSTypeKind.MAP,
            "hash": TSTypeKind.MAP,
        }
    ...
    def _infer_type(self, node: Node) -> TSType:
        ...
        if node_type in ("number", "integer_literal", "float_literal"):
            return TSType(
                TSTypeKind.INT if "." not in self._get_text(node) else TSTypeKind.FLOAT
            )
