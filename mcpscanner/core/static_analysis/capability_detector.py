...
from typing import Any, ClassVar, Dict, Iterator, List, Optional, Set, Tuple, Union, TYPE_CHECKING
    ...
    def _collect_q_registrations(
        ...
        *,
        ...
        for sig, captures in sorted(ordered, key=lambda item: item[0]):
            ...
            reg["registration_snippet"] = self._ts_get_node_text(call_node)
            reg["call_node"] = call_node
            self._ts_enrich_registration_metadata(reg, root)
                    ...
                    if right.type == "expression_list":
                        for ch in right.children:
                            if ch.type == "call_expression":
                                value_node = ch
                                break
                    if self._ts_is_mcp_factory_call(value_node, sdk_aliases):
                        if left.type == "identifier":
                            trusted.add(self._ts_get_node_text(left))
                        else:
                            names = self._ts_expression_list_names(left)
                            if names:
                                trusted.add(names[0])
    ...
    def _ts_resolve_cross_file_endpoint_array(
        ...
        *,
        ...
        for fp, (tree, source_bytes) in files.items():
            ...
            remote_det = CapabilityDetector(remote)
            remote_bindings = remote_det._ts_build_simple_value_bindings(tree.root_node)
            value_node = remote_bindings.get(base_name)
            if value_node is None:
                value_node = remote_det._ts_find_exported_binding(tree.root_node, base_name)
            ...
            if prop_name:
                if value_node.type not in ("object", "object_expression"):
                    continue
                field_val = remote_det._ts_object_field_value_node(value_node, prop_name)
                if field_val is not None and field_val.type in (
                    "array",
                    "array_expression",
                ):
