...
class TaintShape:
    ...
    def __eq__(self, other: object) -> bool:
        ...
        if (
            ...
            or self.is_array != other.is_array
            or self.collapsed != other.collapsed
        ...
        if (
            self.scalar_taint.status != other.scalar_taint.status
            or self.scalar_taint.labels != other.scalar_taint.labels
        ):
            return False
    ...
    def __repr__(self) -> str:
        ...
        if self.is_object:
            fields_str = ", ".join(
                f"{k}: {v.get_taint().status.value}" for k, v in self.fields.items()
            )
            return f"Object({{{fields_str}}})"
        elif self.is_array:
            elem_taint = (
                self.element_shape.get_taint() if self.element_shape else Taint()
            )
