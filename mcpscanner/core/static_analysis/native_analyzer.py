...
# ---------------------------------------------------------------------------
# MCP capability detection lives in ``capability_detector``. The constants,
# regexes, and helpers are re-exported here so callers/tests that historically
# imported them from ``native_analyzer`` keep working without churn.
# ---------------------------------------------------------------------------
from .capability_detector import (  # noqa: E402,F401  (re-export)
    CapabilityDetector,
    CapabilityRecord,
...
class NativeAnalyzer:
    ...
    def _has_mcp_markers(self) -> bool:
        ...
        Lives on :class:`NativeAnalyzer` (rather than
        :class:`CapabilityDetector`) so the behavioral analyzer's
        directory-level prefilter can call it without instantiating a
        detector, and so the per-instance cache survives across
        multiple capability detection passes on the same file.
        """
    ...
    def extract_mcp_capability_contexts(
        ...
        """Backward-compatible shim for capability detection.

        The implementation moved to
        :class:`mcpscanner.core.static_analysis.capability_detector.CapabilityDetector`
        as part of splitting capability-shaped concerns out of the
        general AST/dataflow code. This method instantiates a detector
        bound to ``self`` and forwards the call so existing callers
        (the behavioral code analyzer + the static-analysis test
        suite) keep working unmodified. New code should construct a
        :class:`CapabilityDetector` directly.

        Emits :class:`DeprecationWarning` once per call site so
        downstream integrations can migrate at their own pace.
        """
        import warnings
        ...
        warnings.warn(
            "NativeAnalyzer.extract_mcp_capability_contexts is deprecated; "
            "construct CapabilityDetector(analyzer).detect() (or "
            "CapabilityDetector(analyzer).extract_mcp_capability_contexts() "
            "for the legacy FunctionContext shape) instead.",
            DeprecationWarning,
            stacklevel=2,
        )
        detector = CapabilityDetector(self)
        return detector.extract_mcp_capability_contexts(
