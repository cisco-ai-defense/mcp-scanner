...
class TreeSitterCFGBuilder:
    ...
    ) -> tuple[Optional[TSCFGNode], List[TSCFGNode]]:
    ...
    def _build_if(self, stmt: Node) -> tuple[TSCFGNode, List[TSCFGNode]]:
        ...
        consequence = stmt.child_by_field_name(
            "consequence"
        ) or stmt.child_by_field_name("body")
        if consequence:
        ...
        alternative = stmt.child_by_field_name(
            "alternative"
        ) or stmt.child_by_field_name("else")
        if alternative:
    ...
    def _build_try(self, stmt: Node) -> tuple[TSCFGNode, List[TSCFGNode]]:
        ...
        # Get finally block
        finally_block = stmt.child_by_field_name(
            "finalizer"
        ) or stmt.child_by_field_name("ensure")
        if finally_block:
