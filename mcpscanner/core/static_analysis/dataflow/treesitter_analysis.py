...
class TSFlowFact:
    ...
    def merge(self, other: "TSFlowFact") -> "TSFlowFact":
        ...
        all_params = set(self.parameter_flows.keys()) | set(
            other.parameter_flows.keys()
        )
        for param in all_params:
            ...
            if self_flow and other_flow:
                result.parameter_flows[param] = TSFlowPath(
                    ...
                    operations=self_flow.operations
                    + [
                        op
                        for op in other_flow.operations
                        if op not in self_flow.operations
                    ],
                    reaches_calls=union(
                        self_flow.reaches_calls, other_flow.reaches_calls
                    ),
                    reaches_assignments=union(
                        self_flow.reaches_assignments, other_flow.reaches_assignments
                    ),
                    reaches_returns=self_flow.reaches_returns
                    or other_flow.reaches_returns,
                    reaches_external=self_flow.reaches_external
                    or other_flow.reaches_external,
                )
            elif self_flow:
                result.parameter_flows[param] = TSFlowPath(
                    parameter_name=param,
...
class TreeSitterDataflowAnalysis:
    ...
    def analyze(self) -> List[TSFlowPath]:
        ...
        for param in self.param_names:
            taint = Taint(
                ...
                labels={param},
                sources=[
                    SourceTrace(source_pattern=param, call_site=None, labels={param})
                ],
            )
        ...
        if not target or not value:
            ...
            for child in node.children:
                if child.type == "identifier" and target is None:
                    ...
                    child.type not in ("=", "identifier", ":=", "let", "var", "const")
                    and value is None
                ):
                    value = child
    ...
    def _facts_changed(self, old: TSFlowFact, new: TSFlowFact) -> bool:
        ...
        for param in old.parameter_flows:
            ...
            if (
                old_flow.reaches_returns != new_flow.reaches_returns
                or old_flow.reaches_external != new_flow.reaches_external
                or set(old_flow.reaches_calls) != set(new_flow.reaches_calls)
                or set(old_flow.reaches_assignments)
                != set(new_flow.reaches_assignments)
            ):
                return True
        ...
        for fact in self.facts.values():
            for param, flow in fact.parameter_flows.items():
                if param in result_flows:
                    ...
                    result.reaches_calls = union(
                        result.reaches_calls, flow.reaches_calls
                    )
                    result.reaches_assignments = union(
                        result.reaches_assignments, flow.reaches_assignments
                    )
                    result.reaches_returns = (
                        result.reaches_returns or flow.reaches_returns
                    )
                    result.reaches_external = (
                        result.reaches_external or flow.reaches_external
                    )
