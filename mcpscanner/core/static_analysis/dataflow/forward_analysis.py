...
class ForwardDataflowAnalysis(DataFlowAnalyzer[ForwardFlowFact]):
                        ...
                        for param_name in self.parameter_names:
                            if self._expr_uses_var(node.value, param_name, fact):
                                if param_name in fact.parameter_flows:
                                    fact.parameter_flows[
                                        param_name
                                    ].reaches_assignments.append(
                                    ...
                                    if isinstance(node.value, ast.Call):
                                        ...
                                        fact.parameter_flows[
                                            param_name
                                        ].reaches_calls.append(call_name)
                                        # LLM will determine if operations are dangerous
                    else:
                        # Clear taint
                        fact.shape_env.set_taint(
                            target.id, Taint(status=TaintStatus.UNTAINTED)
                        )
                ...
                if arg_taint.is_tainted():
                    for param_name in self.parameter_names:
                        if self._expr_uses_var(arg, param_name, fact):
                            if param_name in fact.parameter_flows:
                                fact.parameter_flows[param_name].reaches_calls.append(
                                    call_name
                                )
                                fact.parameter_flows[param_name].operations.append(
                                    {
    ...
    def _expr_uses_var(
        self, expr: ast.AST, var_name: str, fact: ForwardFlowFact
    ) -> bool:
        ...
        for fact in facts[1:]:
            ...
            for param_name, flow in fact.parameter_flows.items():
                if param_name in result.parameter_flows:
                    # Merge operations
                    result.parameter_flows[param_name].operations.extend(
                        flow.operations
                    )
                    result.parameter_flows[param_name].reaches_calls.extend(
                        flow.reaches_calls
                    )
                    result.parameter_flows[param_name].reaches_assignments.extend(
                        flow.reaches_assignments
                    )
                    result.parameter_flows[param_name].reaches_returns = (
                        result.parameter_flows[param_name].reaches_returns
                        or flow.reaches_returns
                    )
                    result.parameter_flows[param_name].reaches_external = (
                        result.parameter_flows[param_name].reaches_external
                        or flow.reaches_external
                    )
                else:
