...
class AvailableExprsFact:
    ...
    param_exprs: set[str] = field(
        default_factory=set
    )  # Expressions involving MCP params

    def copy(self) -> "AvailableExprsFact":
        """Create a copy."""
        return AvailableExprsFact(
            available=self.available.copy(), param_exprs=self.param_exprs.copy()
        )
    ...
    def __eq__(self, other: object) -> bool:
        ...
        if not isinstance(other, AvailableExprsFact):
            return False
        return (
            self.available == other.available and self.param_exprs == other.param_exprs
        )
...
class AvailableExpressionsAnalyzer(DataFlowAnalyzer[AvailableExprsFact]):
    ...
    def _transfer_python(self, ast_node: ast.AST, fact: AvailableExprsFact) -> None:
        ...
        if isinstance(ast_node, ast.Assign):
            ...
            fact.available = {
                expr
                for expr in fact.available
                if not self._expr_uses_vars(expr, assigned_vars)
            }
            fact.param_exprs = {
                expr
                for expr in fact.param_exprs
                if not self._expr_uses_vars(expr, assigned_vars)
            }
