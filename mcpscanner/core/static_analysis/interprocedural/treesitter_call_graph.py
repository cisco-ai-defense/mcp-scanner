...
class TSCallGraph:
    ...
    functions: Dict[str, Node] = field(
        default_factory=dict
    )  # full_name -> function node
    ...
    def add_function(
        self, name: str, node: Node, file_path: Path, is_entry: bool = False
    ) -> None:
...
class TreeSitterCallGraphAnalyzer:
    ...
    def _extract_functions(
        self, file_path: Path, root: Node, source_bytes: bytes, class_name: str = ""
    ) -> None:
    ...
    def _decorator_texts_for_function(
        ...
        def visit(node: Node) -> None:
            if node.type == "call_expression":
                ...
                if func is not None and func.type == "member_expression":
                    ...
                    if prop is not None:
                        method = source_bytes[prop.start_byte : prop.end_byte].decode(
                            "utf-8"
                        )
    ...
    def _get_call_name(self, node: Node, source_bytes: bytes) -> str:
        ...
        if node.type == "member_call_expression":
            ...
            if obj is not None and name is not None:
                ...
                name_text = source_bytes[name.start_byte : name.end_byte].decode(
                    "utf-8"
                )
        ...
        for func_name in reachable:
            ...
            for caller, callee in self.call_graph.calls:
                if callee == func_name and (
                    caller == entry_point or caller in param_influenced
                ):
