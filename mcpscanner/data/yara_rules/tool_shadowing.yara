...
rule tool_shadowing{
    ...
    strings:
        ...
        $intercept_calls = /\b(intercept|hijack|reroute|redirect)(s|ing|ed)?\s+(all\s+|any\s+|every\s+)?(calls?|requests?|invocations?|inputs?|arguments?)\s+(to|from|for|intended\s+for)\s+(the\s+)?([\w.-]+\s+){0,4}tools?\b/i
        ...
        $override_other_tool = /\b(override|replace|shadow|supersede|take\s+over)(s|ing|ed)?\s+(the\s+)?(behavio(u)?r|output|response|logic|definition)\s+of\s+(the\s+)?([\w.-]+\s+){0,4}tools?\b/i
